...
如果你想用名字查分數、用 ID 查使用者，用 Vec 當然也做得到——存一堆 `(名字, 分數)` 的 tuple，要查的時候從頭走訪找到名字相符的那個。但這樣每次查詢都要走訪整個 Vec，資料越多越慢。

`HashMap` 解決了這個問題。它用 hash 函數把 key 對應到記憶體位置，不管裡面有多少資料，查一個 key 的速度幾乎是固定的。
