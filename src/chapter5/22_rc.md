...
如果你需要的是「多個人共享**同一份**資料」，`Box` 的 `clone` 就不是正確的工具了。
...
換句話說，Rust 原生的所有權規則就是「一把鑰匙」——這是語言層面的保證，零成本。`Rc` 的「多把鑰匙」是用計數器在執行時期**模擬**出來的，繞過了原生的限制，但也因此有額外的成本和限制。能用 `Box` 就用 `Box`，需要共享時才用 `Rc`。
