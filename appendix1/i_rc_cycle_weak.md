...
理解 `Rc` 引用迴圈會造成記憶體洩漏，並學會用 `Weak` 來打破迴圈。

> 本集是**第五章**的補充。

## 概念說明

還記得第五章學的 `Rc<T>` 嗎？它透過引用計數來管理記憶體——每多一個 `Rc` 指向同一筆資料，計數就加一；每少一個就減一；歸零時釋放記憶體。

聽起來很完美，但有一個致命弱點：**引用迴圈（reference cycle）**。
...
`Weak<T>` 是一種「弱引用」——它指向同一筆資料，但**不會增加強引用計數（strong count）**。這意味著 `Weak` 不會阻止資料被釋放。
...
- `Rc` 的引用迴圈會造成記憶體洩漏——計數永遠不會歸零，記憶體永遠不會釋放
