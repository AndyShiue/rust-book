...
第四章講借用的時候，我們留了一個伏筆：「不能回傳區域變數的引用。」現在來正式面對這個問題。
...
fn make_greeting() -> &str {
    ...
    &s // 編譯錯誤！
} // s 在這裡被釋放了，回傳的引用指向一塊已經不存在的記憶體
```

這個比較好理解——`s` 離開函數就沒了，回傳它的引用毫無意義。Rust 直接擋掉。
...
這段程式碼也會編譯失敗。`a` 和 `b` 都是外面傳進來的引用，不會在函數結束時消失，那為什麼不行？

因為 Rust 在檢查**呼叫端**的時候，需要知道回傳值的引用能「活多久」。看這個例子：
...
如果 `longer` 回傳了 `a`（也就是 `&s1`），`result` 是安全的，因為 `s1` 還活著。但如果回傳了 `b`（也就是 `&s2`），`result` 就是懸垂引用——`s2` 已經被釋放了。
...
可變引用也可以加生命週期標注，寫成 `&'a mut T`——就是把 `'a` 放在 `&` 和 `mut` 之間。`'a` 一樣描述這個引用能活多久。
...
**重要觀念**：生命週期標注不會讓任何引用活得更久或更短。它只是**描述**已有的關係，幫助編譯器做檢查。就像型別標注不會改變值的內容一樣。

### 不是所有函數都要標

如果函數只有一個引用參數，Rust 能自動推斷（下一集會詳細講）：
