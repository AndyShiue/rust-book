...
## 本集目標
學會用 `Rc<T>` 讓多個擁有者共享同一份資料，理解引用計數的原理。
...
對 Rc 呼叫 `.clone()` 和我們之前學的 `.clone()` 不一樣——它不會複製裡面的資料，只是增加引用計數。所以速度很快，成本很低。
...
`Rc` 在 heap 上放一個 struct，裡面有計數器和資料。`.clone()` 只是讓計數器 +1，多一個指標指向同一塊記憶體。Drop 時計數 -1，歸零才釋放。
...
## 重點整理
- `Rc<T>` 用引用計數讓多個擁有者共享同一份資料
- Rc 的 `.clone()` 只增加計數，不複製資料——成本很低
- 某個 Rc 離開作用域時計數 -1，歸零時才釋放資料
- Rc 是**唯讀的**——所有擁有者只能讀，不能改
- 用 `Rc::strong_count(&x)` 查看目前的引用計數
