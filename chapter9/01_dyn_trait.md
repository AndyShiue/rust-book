...
但既然不知道具體是什麼，`dyn Describe` 的大小就不固定——Cat 可能佔 1 byte，Dog 可能佔 100 bytes，編譯器在編譯期不知道會是哪個。所以 `dyn Describe` 是 DST（附錄一第 k 集學過），必須放在指標後面：
...
附錄一第 k 集學過 `&[T]` 是胖指標（位址 + 長度）。`&dyn Trait` 也是胖指標，但存的東西不同：
...
`Box<dyn Trait>` 完整寫法是 `Box<dyn Trait + 'static>`——Rust 預設幫你補上 `'static`。`Trait + 'static` 是一個 trait bound——跟你在泛型裡寫 `T: Trait + 'static` 的意思一樣。`dyn` 把這個 bound 變成一個型別。所以 `dyn Trait + 'a` 讀成 `dyn (Trait + 'a)`。

`+ 'static` 代表裡面裝的具體型別不能包含任何非 `'static` 的參考：
...
let foo = Foo(&s);
// let boxed: Box<dyn Describe> = Box::new(foo);
// 編譯錯誤！Foo 借用了 s，s 不是 'static
```

如果需要裝有借用的型別，明確寫出 lifetime：
...
`&'a dyn Trait` 則預設是 `&'a (dyn Trait + 'a)`——不用特別處理。
...
- `Box<dyn Trait>` 預設隱含 `+ 'static`；`dyn Trait + 'a` 讀成 `dyn (Trait + 'a)`，dyn 把 trait bound 變成型別
