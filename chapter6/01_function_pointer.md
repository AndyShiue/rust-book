...
在 Rust 裡，每個函數都有自己獨特的型別。但除了這個獨特型別之外，函數還可以被「轉換」成一個通用的**函數指標型別**。
...
函數指標 `fn(...)` 是一個具體的型別，大小固定（就是一個記憶體位址）。下一集會介紹閉包（closure），它比函數指標更強大，因為它能「捕捉」外部變數。但今天先把函數指標搞清楚——它是閉包的基礎。
...
- 函數名稱可以直接當成值，賦值給變數或傳遞給其他函數
- 函數指標的大小固定（一個記憶體位址），可以存進 Vec 等容器
- 只要簽名相同的函數，就能互相替換
- 函數指標是閉包的基礎，下一集會介紹更強大的閉包
